...
# tell flask where/which server to send the email to
# mailtrap.io (free one for testing)
"""
...
def register():
    ...
    if test:
        ...
        db.session.add(user)
        db.session.commit()
        # 201 = created a recprd successfully
...
def login():
    ...
    if test:
        # identity = email id used to identify the user
